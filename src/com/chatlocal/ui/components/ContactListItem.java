...
public class ContactListItem extends JPanel {
    ...
    private void showContextMenu(int x, int y, ContactActionCallback callback) {
        ...
        menu.setBackground(ThemeManager.getTheme().bgCard);
        menu.setBorder(BorderFactory.createLineBorder(ThemeManager.getTheme().borderSubtle, 1));
        ...
        itemBlock.setBackground(ThemeManager.getTheme().bgCard);
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (selected) {
            g2.setColor(ThemeManager.getTheme().bgCardHover);
            g2.fillRoundRect(4, 3, w - 8, h - 6, r, r);
            g2.setColor(ThemeManager.getTheme().primary);
            g2.fillRect(4, 12, 3, h - 24);
        } else if (hovered) {
            g2.setColor(new Color(ThemeManager.getTheme().bgCardHover.getRed(),
                    ThemeManager.getTheme().bgCardHover.getGreen(),
                    ThemeManager.getTheme().bgCardHover.getBlue(), 120));
            g2.fillRoundRect(4, 3, w - 8, h - 6, r, r);
        }
        ...
        if (isRoom) {
            Icon icon = isMeeting ? Icons.group(22, ThemeManager.getTheme().primary) : Icons.group(20, ThemeManager.getTheme().textSecondary);
            ...
            if (isOnline) {
                g2.setColor(ThemeManager.getTheme().success);
        ...
        g2.setColor(selected ? ThemeManager.getTheme().textPrimary : ThemeManager.getTheme().textPrimary);
        String displayTitle = truncate(g2, title, maxTextW);
        g2.drawString(displayTitle, textX, 24);

        // Subtítulo
        g2.setFont(new Font("Segoe UI", Font.PLAIN, 11));
        g2.setColor(ThemeManager.getTheme().textMuted);
