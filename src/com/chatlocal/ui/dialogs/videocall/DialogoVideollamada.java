...
import com.chatlocal.backend.service.videocall.TipoFuenteVideo;
...
public class DialogoVideollamada extends JDialog implements OyenteVideollamada {

    private final ServicioVideollamada callService;
    ...
    private BotonModerno btnMic;
    private BotonModerno btnVideo;
    private BotonModerno btnScreen;
    private JComboBox<TipoFuenteVideo> comboSources;
    ...
    public DialogoVideollamada(Frame parent, ServicioVideollamada callService, String peerName) {
    ...
    private void buildUI() {
        ...
        remoteVideoPanel = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                if (mainImg != null && (swapViews || callService.getCallState() == EstadoLlamada.CONNECTED)) {
        ...
        localPipPanel = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                g2.setColor(activeAudio > 0.05f ? new Color(16, 185, 129) : GestorTema.getTheme().borderActive);
    ...
    private JPanel createTopBar() {
        ...
        JLabel lblAvatar = new JLabel(Iconos.avatar(peerName, 32, GestorTema.getTheme().primary, Color.WHITE));
        left.add(lblAvatar);
        ...
        comboSources = new JComboBox<>(TipoFuenteVideo.values());
        comboSources.setSelectedItem(callService.getVideoSourceType());
        comboSources.setFont(new Font("Segoe UI", Font.PLAIN, 11));
        comboSources.setBackground(GestorTema.getTheme().bgSidebar);
        comboSources.setForeground(GestorTema.getTheme().textPrimary);
        comboSources.setFocusable(false);
        comboSources.addActionListener(e -> {
            TipoFuenteVideo selected = (TipoFuenteVideo) comboSources.getSelectedItem();
            if (selected != null) {
    ...
    private JPanel createControlDock() {
        ...
        // 1. Micrófono
        btnMic = new BotonModerno("", Iconos.mic(18, Color.WHITE), BotonModerno.Variant.GHOST);
        ...
        // 2. Cámara
        btnVideo = new BotonModerno("", Iconos.video(18, Color.WHITE), BotonModerno.Variant.GHOST);
        ...
        btnScreen = new BotonModerno("", Iconos.screenShare(18, Color.WHITE), BotonModerno.Variant.GHOST);
        ...
        btnScreen.addActionListener(e -> {
            if (callService.getVideoSourceType() == TipoFuenteVideo.SCREEN_SHARE) {
                callService.setVideoSourceType(TipoFuenteVideo.VIRTUAL_CAMERA);
                comboSources.setSelectedItem(TipoFuenteVideo.VIRTUAL_CAMERA);
            } else {
                callService.setVideoSourceType(TipoFuenteVideo.SCREEN_SHARE);
                comboSources.setSelectedItem(TipoFuenteVideo.SCREEN_SHARE);
            }
            updateControlButtons();
        });

        // 4. Colgar (Rojo)
        BotonModerno btnHangup = new BotonModerno("", Iconos.phoneEnd(20, Color.WHITE), BotonModerno.Variant.DANGER);
    ...
    private void updateControlButtons() {
        ...
        btnMic.setIcon(isMuted ? Iconos.micOff(18, Color.WHITE) : Iconos.mic(18, Color.WHITE));
        btnMic.setBackground(isMuted ? GestorTema.getTheme().danger : GestorTema.getTheme().bgCard);

        boolean isVideoOn = callService.isVideoEnabled();
        btnVideo.setIcon(isVideoOn ? Iconos.video(18, Color.WHITE) : Iconos.videoOff(18, Color.WHITE));
        btnVideo.setBackground(!isVideoOn ? GestorTema.getTheme().danger : GestorTema.getTheme().bgCard);

        boolean isSharing = callService.getVideoSourceType() == TipoFuenteVideo.SCREEN_SHARE;
        btnScreen.setBackground(isSharing ? new Color(16, 185, 129) : GestorTema.getTheme().bgCard);
    }
    ...
    private void drawWaitingPlaceholder(Graphics2D g2, int w, int h) {
        ...
        String title = (callService.getCallState() == EstadoLlamada.OUTGOING_CALL)
                ? "Llamando a " + peerName + "..."
    ...
    private void startTimer() {
        ...
        durationTimer = new javax.swing.Timer(1000, e -> {
            if (callService.getCallState() == EstadoLlamada.CONNECTED) {
    ...
    public void onCallStateChanged(EstadoLlamada newState, String peerName, String message) {
        SwingUtilities.invokeLater(() -> {
            ...
            if (newState == EstadoLlamada.CONNECTED) {
                callStartTime = System.currentTimeMillis();
